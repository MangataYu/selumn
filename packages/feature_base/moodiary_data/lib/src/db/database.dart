        ...
        () => m.runMigrationSteps(
          ...
          steps: migrationSteps(
            ...
            from3To4: (m, schema) async {
              // personal 测试版和上游助手改版都使用过 v3，按实际列兼容两种结构。
              if (!await _hasColumn('chat_messages', 'provider_id')) {
                await m.addColumn(
                  schema.chatMessages,
                  schema.chatMessages.providerId,
                );
              ...
              await customStatement('DROP TABLE IF EXISTS agent_presets');
              for (final column in [
                'agent_preset_id',
                'persona_snapshot',
                'tools_snapshot_json',
              ]) {
                if (await _hasColumn('chat_sessions', column)) {
                  await m.dropColumn(schema.chatSessions, column);
                }
              }
            },
