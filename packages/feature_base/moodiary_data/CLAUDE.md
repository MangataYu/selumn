...
- Legacy category-to-tag conversion runs after a complete import/pull batch and at startup. It clears `categoryId` only after resolving the ancestor path; missing parents and cycles retain the old association for retry. While that association is pending, `legacyCategoryExcludedTags` / `legacy_category_excluded_tags_json` records explicit tag removals and renames so a late parent cannot restore a removed tag. Exclusions match a path and its descendants, block only automatic additions, and are cleared together with `categoryId` when migration completes. Preserve this field in sync and backup payloads; older payloads default to an empty list. Explicitly adding an exact excluded path removes that exclusion, but adding a child does not cancel an excluded parent.
