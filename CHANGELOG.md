...
从本版本起使用独立的 Selume 版本号；下方 2.x 条目保留 Moodiary 的上游历史。
...
- 将历史数据迁移进度与应用版本分开记录，避免启用独立版本号后重复执行旧迁移。
- 关于页的源码和反馈入口改为 Selume 仓库，移除原项目的备案号，保留上游署名与第三方开源许可。

### 安装与更新

- 适用于 Android 9 及以上的 ARM64 手机；下载 `selume-1.0.0-android-arm64.apk` 后打开安装，按系统提示允许浏览器安装应用。
- 后续可在「设置 → 关于 → 检查更新」查看新版本，再通过浏览器下载 APK 并安装。
- 覆盖升级使用相同包名和签名，保留应用数据；无需先卸载旧版。
- 从旧 2.x 测试版切换到 Selume 1.0.0 时，需手动安装一次 APK；Android 内部构建号继续递增为 97。
